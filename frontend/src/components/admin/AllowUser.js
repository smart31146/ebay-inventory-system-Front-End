...
import { setDeactiveUsers } from "../../redux/actions/productActions";

export default function AllowUser() {
    const user = JSON.parse(localStorage.getItem('user'));
    const users = useSelector(state => state.product.deactive_users);
    ...
    function updateUsers() {
        endpoints.get_deactive_userlist().then(res => {
            dispatch(setDeactiveUsers(res.data));
        })
            .catch(error => {
                            ...
                            <tr key={i}>
                                ...
                                <td>
                                    <div className="form-check form-switch">
                                        <input className="form-check-input" type="checkbox" style={{width: 60, height: 30}} id="mySwitch" checked={false} name="darkmode" onChange={(e) => allow_user(v.id, e)} />
                                    </div>
                                </td>
                                <td>
